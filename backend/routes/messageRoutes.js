...
router.post('/:id/like', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Beğeni işlemi başarısız oldu' });
  }
});
...
router.get('/yemek-degerlendirme/kullanici/:kullanici_id', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Kullanıcı değerlendirmeleri alınamadı' });
  }
});
...
router.delete('/:messageId/comments/:commentId', async (req, res) => {
  try {
    ...
    // Yorumu sil
    await db.query(
      'DELETE FROM mesaj_yorumlar WHERE id = ?',
      [commentId]
    );

    // Yorum beğenilerini de sil
    await db.query(
      'DELETE FROM mesaj_yorum_begeniler WHERE yorum_id = ?',
      [commentId]
    );
